...
const TICKER = ["?? 6 AI Agents","? Real-time","?? 150+ Charts","?? AutoML","??? Secure","?? GPT-4","?? Live Dashboards","?? Anomaly Detection"];
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden dot-grid scanline" style={{ background: "#09090b", paddingTop: 40 }}>
      {/* Orbs */}
      <div className="fixed rounded-full pointer-events-none" style={{ width:600,height:600,top:"-10%",left:"-5%",background:"radial-gradient(circle,rgba(250,204,21,.1) 0%,transparent 65%)",animation:"orbFloat 14s ease-in-out infinite" }} />
      <div className="fixed rounded-full pointer-events-none" style={{ width:500,height:500,bottom:"-8%",right:"-5%",background:"radial-gradient(circle,rgba(245,158,11,.08) 0%,transparent 65%)",animation:"orbFloat 18s ease-in-out 2s infinite" }} />
      ...
      {/* Card */}
      <div className="fade-up relative z-10 w-full" style={{ maxWidth:440,background:"rgba(14,14,16,.92)",border:"1px solid rgba(255,255,255,.08)",borderRadius:20,padding:"32px 36px 26px",boxShadow:"0 0 60px rgba(250,204,21,.08),0 40px 80px rgba(0,0,0,.7)" }}>
        {/* Logo */}
        <div className="flex items-center justify-center gap-3 mb-1">
          <div style={{ width:42,height:42,borderRadius:12,background:"linear-gradient(135deg,#FACC15,#F59E0B)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(250,204,21,.5)" }}><Brain size={22} color="#000" strokeWidth={2.5} /></div>
        ...
        {/* Demo Credentials */}
        <div style={{ marginBottom:16,padding:"10px 12px",borderRadius:8,background:"rgba(250,204,21,.08)",border:"1px solid rgba(250,204,21,.2)" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
            <div>
              <p style={{ fontSize:10,fontWeight:700,color:"#FACC15",letterSpacing:"0.08em",marginBottom:6 }}>?? DEMO CREDENTIALS</p>
              <p style={{ fontSize:11,color:"#F59E0B",margin:"2px 0" }}>?? rudraadmin@gmail.com</p>
              <p style={{ fontSize:11,color:"#F59E0B",margin:"2px 0" }}>?? adminrudra@1234</p>
            </div>
        ...
        <div style={{ overflow:"hidden",marginBottom:18,maskImage:"linear-gradient(to right,transparent,black 10%,black 90%,transparent)",WebkitMaskImage:"linear-gradient(to right,transparent,black 10%,black 90%,transparent)" }}>
          <div style={{ display:"flex",gap:8,width:"max-content",animation:"ticker 14s linear infinite" }}>
            {[...TICKER,...TICKER].map((t,i) => <span key={i} style={{ fontSize:10,fontWeight:600,color:"#F59E0B",background:"rgba(250,204,21,.08)",border:"1px solid rgba(250,204,21,.18)",borderRadius:20,padding:"3px 10px",whiteSpace:"nowrap" }}>{t}</span>)}
          </div>
        </div>
          ...
          {["login","register"].map(m => (
            <button key={m} type="button" onClick={() => { setMode(m); setErr(null); }}
              style={{ flex:1,padding:"9px",borderRadius:9,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",transition:"all .2s",
                background: mode===m ? "linear-gradient(135deg,#FACC15,#F59E0B)" : "transparent",
                color: mode===m ? "#fff" : "#52525b",
                boxShadow: mode===m ? "0 2px 16px rgba(250,204,21,.4)" : "none" }}>
        ...
        <form onSubmit={submit} style={{ display:"flex",flexDirection:"column",gap:10 }}>
          ...
          {mode === "login" && (
            <label style={{ display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:"#71717a" }}>
              <input type="checkbox" checked={rememberMe} onChange={e=>setRememberMe(e.target.checked)} style={{ accentColor:"#F59E0B" }} />
              Remember me
            </label>
          )}
          ...
          {mode === "login" && (
            <button type="button" onClick={() => { setShowForgot(true); setForgotMsg(null); setForgotEmail(email); }}
              style={{ background:"transparent",border:"none",cursor:"pointer",color:"#FACC15",fontSize:12,textAlign:"left",padding:0,textDecoration:"underline" }}>
              Forgot password?
            </button>
          )}
          ...
          {mode === "register" && (
            <label style={{ display:"flex",alignItems:"flex-start",gap:8,cursor:"pointer",fontSize:12,color:"#71717a" }}>
              <input type="checkbox" checked={tosAccepted} onChange={e=>setTosAccepted(e.target.checked)} style={{ accentColor:"#F59E0B",marginTop:2,flexShrink:0 }} />
              I agree to the <span style={{ color:"#FACC15",textDecoration:"underline",cursor:"pointer" }}>Terms of Service</span> and <span style={{ color:"#FACC15",textDecoration:"underline",cursor:"pointer" }}>Privacy Policy</span>
            </label>
          )}

          <button type="submit" disabled={loading}
            style={{ padding:"13px",borderRadius:11,border:"none",background:"linear-gradient(90deg,#FACC15,#F59E0B)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 4px 24px rgba(250,204,21,.4)",letterSpacing:"0.02em" }}>
      ...
      {requires2fa && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100 }}>
          <div style={{ background:"rgba(14,14,16,.95)",border:"1px solid rgba(250,204,21,.3)",borderRadius:16,padding:"32px 36px",width:340 }}>
            ...
            <form onSubmit={verify2fa} style={{ display:"flex",flexDirection:"column",gap:10 }}>
              ...
              <button type="submit" disabled={loading||totpCode.length!==6}
                style={{ padding:"12px",borderRadius:10,border:"none",background:"linear-gradient(90deg,#FACC15,#F59E0B)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer" }}>
      ...
      {showForgot && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100 }}>
          <div style={{ background:"rgba(14,14,16,.95)",border:"1px solid rgba(250,204,21,.3)",borderRadius:16,padding:"32px 36px",width:360 }}>
            ...
            {!forgotMsg?.ok && (
              <form onSubmit={submitForgot} style={{ display:"flex",flexDirection:"column",gap:10 }}>
                ...
                <button type="submit" disabled={forgotLoading}
                  style={{ padding:"12px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#F59E0B,#FDE047)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>
